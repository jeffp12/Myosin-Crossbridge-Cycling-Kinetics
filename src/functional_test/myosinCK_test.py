...
class TestCrossBridgeDynamics(unittest.TestCase):
    ...
    def test_simulating_it(self):
        ...
        time_end = 0.3
        num_points = 500
        m = self.MODEL.simulate(0, time_end, num_points)
    ...
    def test_initial_concentrations(self):
      # Verify initial conditions. CHANGING THIS IN CODE WILL NEED TO BE CHANGED HERE FOR TEST
        self.biochemrxn()  
        self.assertAlmostEqual(self.MODEL["CB0"], 0.478)
        self.assertAlmostEqual(self.MODEL["CB1"], 0.0)
        self.assertAlmostEqual(self.MODEL["CB2"], 0.014)
        self.assertAlmostEqual(self.MODEL["CB3"], 0.143)
        self.assertAlmostEqual(self.MODEL["CB4"], 0.0)
        self.assertAlmostEqual(self.MODEL["CB5"], 0.144)
        self.assertAlmostEqual(self.MODEL["CB6"], 0.221)
    ...
    def test_concentrations_total(self):
       # Again these are proportion to 1 so all the concentrations at any given time point must still yield 1.
        self.biochemrxn()  
        m = self.MODEL.simulate(0, 0.3, 500)
